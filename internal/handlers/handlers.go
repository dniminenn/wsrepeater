...
var (
	...
	windSpeedValues      []float64
	uvMutex              sync.Mutex
	solarMutex           sync.Mutex
	windSpeedMutex       sync.Mutex
...
func ConvertAndForward(w http.ResponseWriter, r *http.Request) {
	...
	// Smooth UV, solar radiation, and wind speed values
	...
	smoothedWindSpeed := utils.SmoothValue(windSpeedValue, &windSpeedValues, &windSpeedMutex)

	// Correct for overestimation
	correctedUV := math.Round(smoothedUV * 0.92)
	correctedSolarRadiation := smoothedSolarRadiation * 0.85
	...
	wundergroundData.Set("windspeedmph", fmt.Sprintf("%.2f", smoothedWindSpeed))
	...
	// Update latest data asynchronously
